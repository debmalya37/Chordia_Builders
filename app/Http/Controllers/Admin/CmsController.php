...
public function cms(){
$cmspage = Cms::orderBy('sort_order', 'ASC')->get();
return view('admin.pages.cms',compact('cmspage'),['head' => 'Cms']);
}
...
if(!empty($get_record)){
return view('admin.pages.addcms',compact('get_record'),['head' => 'Edit Cms']);
}else{
...
$validator = Validator::make($request->all(),[
'title'         => 'required',
'slug_url' 		=> 'required|unique:cms,slug_url,' . $request->id,
'image'         => 'mimes:jpeg,jpg,png,gif,webp'
]); 
if(!$validator->passes()){
return response()->json(['status'=>0, 'error'=>$validator->errors()->toArray()]);
