<form method="POST" enctype="multipart/form-data" class="upload_image_form" action="javascript:void(0)" >
    ...
    <input type="hidden" data-service="{{$data->event_id}}" value="{{$data->id}}" name="event_id">
    ...
    @foreach($eventimages as $records)
    <tr id="mdels{{ $records->id }}">
    <th scope="row">{{$loop->iteration}}</th>
    <td>{{ $records->title }}</td>
    <td>
    @if(!empty($records->image))
    <img width="50" src="<?php echo asset("event_more_images/$records->image")?>">
    @else
    <img width="80" src="<?php echo asset("images/no-image-available.jpg")?>"> 
    @endif
    </td>
    <td>
    <a id="delete_single" data-url="event/imgdelete" data-id="{{ $records->id }}" data-token="{{ csrf_token() }}"> <i class="btn btn-danger typcn typcn-delete"></i></a></td>
    </tr>
    @endforeach
    @else
    <tr id="trhides">
  ...
  $(this).next('.custom-file-label').html(e.target.files[0].name);
   ...
   $.ajaxSetup({
    ...
    var url = "<?php echo asset("event_more_images")?>";
    $.ajax({
    type:'POST',
    url: "{{ route('event.addimages')}}",
    data: formData,
    cache:false,
    contentType: false,
    processData: false,
    ...
    alert(data.msg);
    var html = '<tr id="mdels'+data.datas.id+'">';
     html += '<td>'+data.datas.id+'</td>';
     html += '<td>'+data.datas.title+'</td>';
     html += '<td><img width="50" src='+url+'/'+data.datas.image+'></td>';
     html += '<td><a id="delete_single" data-url="event/imgdelete" data-id="'+data.datas.id+'"> <i class="btn btn-danger typcn typcn-delete"></i></a></td></tr>';
     $('#imd').prepend(html);
