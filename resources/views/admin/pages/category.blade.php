...
<h3 class="card-title">{{$head}}</h3>
 ...
 <button type="button" onclick="window.location='{{ route('admin.category.create')}}'" class="btn btn-success float-right mb-2" title="Add"><i class="typcn typcn-plus"></i> </button>
</div>
</div>
    <div class="row">
    <div class="col-sm-12">
    ...
    <td>{{ $record->title }}</td>
     <td><input data-id="{{$record->id}}" data-url="category/changeStatus" data-table="tour_categories" class="toggle-class" type="checkbox" data-onstyle="success" data-offstyle="danger" data-toggle="toggle" data-on="Active" data-off="InActive"   <?php echo $record->status ? 'checked' : '';?> ></td>
