<form method="POST" enctype="multipart/form-data" class="upload_image_form" action="javascript:void(0)" >
    ...
    <input type="hidden" data-service="{{$data->project_id}}" value="{{$data->id}}" name="project_id">
    ...
    <tbody id="imd">
    @if(!$projectamenities->isEmpty())
    @foreach($projectamenities as $records)
    <tr id="dels{{ $records->id }}">
    <th scope="row">{{$loop->iteration}}</th>
    <td>{{ $records->title }}</td>
    <td>
    @if(!empty($records->image))
    <img width="50" src="<?php echo asset("amenities_more_images/$records->image")?>">
    @else
    <img width="80" src="<?php echo asset("images/no-image-available.jpg")?>"> 
    @endif
    </td>
    <td>
    <a id="delete_single" data-url="project/amenitiesdelete" data-id="{{ $records->id }}" data-token="{{ csrf_token() }}"> <i class="btn btn-danger typcn typcn-trash"></i></a></td>
    </tr>
    @endforeach
    @else
    <tr id="trhides">
  ...
  $(this).next('.custom-file-label').html(e.target.files[0].name);
   ...
   $.ajaxSetup({
    ...
    var url = "<?php echo asset("amenities_more_images")?>";
    $.ajax({
    type:'POST',
    url: "{{ route('admin.project.addamenities')}}",
    data: formData,
    cache:false,
    contentType: false,
    processData: false,
    ...
    var html = '<tr id="dels'+data.datas.id+'">';
     ...
     html += '<td><a id="delete_single" data-url="porject/amenitiesdelete" data-id="'+data.datas.id+'"> <i class="btn btn-danger typcn typcn-trash"></i></a></td></tr>';
     $('#imd').prepend(html);
